...
interp :: Expr -> Int
interp (Lit n) = n
interp (Op op e1 e2) = (interpBinOp op) (interp e1) (interp e2)
...
peephole optimize (Op op e1 e2) =
  let optimized = optimize (Op op (peephole optimize e1) (peephole optimize e2))
   in if optimized == (Op op e1 e2) then optimized else peephole optimize optimized
peephole optimize e = optimize e
...
step (IPush n) stack = Just (n : stack)
step (IOp op) (x : y : stack) = Just ((interpBinOp op) x y : stack)
